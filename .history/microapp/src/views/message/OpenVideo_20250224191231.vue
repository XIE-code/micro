        ...
        <div class="body_box">
            <div class="body_left_box">
                <div class="choose_box">
                    <div class="choose_image_box">
                        <img
                            src="@/assets/img/home_full.png"
                            alt=""
                            @click="goHome"
                        />
                        <!-- <div class="choose_text">首页</div> -->
                        <router-link
                            to="/web/school/web/main"
                            class="choose_text"
                            >首页</router-link
                        >
                    </div>
            ...
            <div class="body_right_box">
                <div class="body_right_content_box">
                        ...
                        <div class="video_bottom">
                            <div class="video_bottom_item_text">
                                <img
                                    ...
                                    alt="点赞"
                                    @click="thumbsUp"
                                    v-if="isThumbsUp == false"
                                />
                                <img
                                    src="@/assets/img/true-thumbs-up.png"
                                    alt="点赞"
                                    class="text"
                                    @click="thumbsUp"
                                    v-if="isThumbsUp == true"
                                />
                                {{ thumbsUpCount }} 点赞
                            </div>
                            <!-- 下架视频按钮 -->
                            <button
                                class="down_video"
                                @click="toggleConfirmUnshelfModal('unshelf')"
                                v-if="canUnshelf && this.state === 1"
                            >
                                下架视频
                            </button>
                            <!-- 重新上架视频按钮 -->
                            <button
                                class="down_video"
                                @click="toggleConfirmUnshelfModal('reshelf')"
                                v-if="canUnshelf && this.state === 0"
                            >
                                重新上架视频
                            </button>
                            <!-- <div class="video_bottom_item_text">
                    ...
                    <div
                        class="confirm-unshelf-modal"
                        v-if="confirmUnshelfModalVisible"
                        @click.self="cancelUnshelf"
                    >
                        <div class="confirm-unshelf-content" @click.stop>
                            <!-- 根据action显示不同的确认信息 -->
                            <p v-if="action === 'unshelf'">
                                确定要下架视频吗？
                            </p>
                            <p v-if="action === 'reshelf'">
                                确定要重新上架视频吗？
                            </p>
...
export default {
    ...
    methods: {
        async initVideoPage() {
            let videoObj = UtilHelper.getObjectFromCookie(
                CookieUtil.videoObject
            );
            ...
            this.state =
                videoObj.state === undefined ? this.state : videoObj.state;

            this.checkUnshelfPermission();
            const that = this;
            //是否点赞的接口
            await axios
                .get(`${CONNECT.generalhostpost()}/user/is/like/history`, {
                    params: {
        ...
        async unshelfVideo(status) {
            ...
            try {
                const response = await axios.put(
                    `${CONNECT.generalhostpost()}/update/video/status`,
                    null,
                    {
                        params: {
                            videoNumber: that.videoNumber,
        ...
        //去个人页面的方法
        goPerson() {
            UtilHelper.setObjectInCookie(
                CookieUtil.videoOwnerPersonal,
                { account: `${this.accountByVideoOwner}` },
                CookieUtil.userDefaultTime
            );
        ...
        thumbsUp() {
            ...
            axios
                .post(
                    `${CONNECT.generalhostpost()}/user/click/like/history`,
                    requestParams,
                    {
                        headers: {
                            token: that.token,
        ...
        onVideoEnded() {
            const storedArrayString = localStorage.getItem(
                CookieUtil.videoList
            );
...
.parent-web
    ...
    .body_right_box
    .body_right_head_box
    .body_right_head_text_box:nth-child(2) {
    color: #8f9bb3;
}
.parent-web
    .body_box
    .body_right_box
    .body_right_head_box
    .body_right_head_text_box:nth-child(1) {
    color: #000000;
}
